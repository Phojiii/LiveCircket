...
function flattenRecentMatches(typeMatches = []) {
  const matches = [];
...
export default function Recent({ data }) {
  const matches = flattenRecentMatches(data?.typeMatches);

  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-yellow-200 py-10">
      <div className="max-w-5xl mx-auto px-4">
        <header className="mb-10 text-center">
          ...
          <p className="text-lg text-gray-600">
            All recently finished international, league, domestic, and women's matches.
          </p>
        </header>
        {matches.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {matches.map((match, idx) => (
              <div
                ...
                className="bg-white rounded-lg shadow-md p-6 border border-yellow-100 hover:shadow-xl transition"
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    {match.team1?.imageId ? (
                      ...
                      <span className="w-7 h-7 bg-yellow-100 rounded-full flex items-center justify-center text-yellow-700 font-bold">
                        {match.team1?.teamSName?.slice(0, 2) || "T1"}
                      </span>
                    )}
                    <span className="font-semibold text-yellow-700">{match.team1?.teamSName}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-semibold text-yellow-700">{match.team2?.teamSName}</span>
                  </div>
                </div>
                ...
                <div className="text-yellow-900 font-bold text-base">
